...
%%% Remove temporarily the names and affiliations of authors from the title page.
...
Within the framework of this study the suitability of the \emph{Rust} ecosystem
for forensic tool development is evaluated.  The development of a forensic
tool named \emph{Strings­ext}\cite{getreu2016a}\cite{getreu2017} serves as a
case study.  Starting from analysing the specific requirements of forensic
software in general and those of the present case study, all stages of the
software development life-cycle have been executed, up to the first
production release.  \emph{Strings­ext} is a reimplementation and
enhancement of the \emph{GNU-strings} tool, a widely used program in
forensic investigations.  \emph{Strings­ext} recognizes Cyrillic, CJKV
...
information constituting digital evidences. 

In the domain of digital forensics an observation of a well-known
cause-effect-relationship between an electronic trace (effect) and what has
happened (cause) is a so called \textbf{artefact}. It embodies any ``item of
interest that helps an investigation move forward.'' \footnote{
...
analyse how similar tools are verified. Beckett and Slay \cite{beckett2007}
suggest a model so called \emph{Model of tool neutral testing} or
\emph{functionality oriented validation}. Instead of testing if a software
product meets all its requirements, an independent set of forensic functions and
their specifications is defined. This allows to decouple the validation
procedure from the implementation of the forensic tool itself. A forensic
function is an activity required in forensic investigation that produces known
valid results for a given set of test cases.  For example, Figure
\ref{the-search-target-mapping} \cite[17]{beckett2007} shows the search target
mapping, a property of the general \emph{search function}. The subcategory
``Character encoding'' illustrates the main deficit of \emph{GNU-strings} as it
supports only ASCII encoding. In global cyberspace, forensic tools must identify
a multitude of encodings. This leads us to the main motivation and requirement
of \emph{Strings­ext}: \emph{multibyte character encoding support}.
...
acceptable costs. This requires test cases with known valid results. With
...
with the \emph{test driven development} method is chosen for the
...
widely cited when it comes to define anti-forensics. This traditional 
"hide-and-seek" relation might soon take a new dimension: Eggendorfer
...
still largely neglected despite its potential impact:
...
\emph{GNU strings} addresses this risk with the programming language
Rust. Rust offers some outstanding security guarantees which are presented
in Section \ref{rust-lang}.
...
Chapter \ref{rust-lang} expends on how Rust meets these requirements by its
...
\begin{figure}[htbp]
   \centering
   \includegraphics[height=0.20000\textwidth]{images/test_case_encoding-input.png}
   \caption{Test case international character encodings}
   \label{fig:test-international-character-encodings}
\end{figure}

The above file is then is converted into \texttt{UTF-8}, \texttt{UTF-16LE},
\texttt{UTF-16BE}, \texttt{UTF-32LE} and \texttt{UTF-32BE}, in each case one
encoding per file.  In order to observe \emph{GNU-strings} Unicode detection
capabilities, all the test-files are then searched for valid graphic strings
with the command \texttt{strings} using all possible variation of its encoding
filter.  The Figure \ref{fig:strings-e-l} shows \emph{GNU-strings} output for an
UTF-16 little endian encoded input as an example.
 

\begin{figure}[htbp]
\centering
\includegraphics[height=0.20000\textwidth]{images/strings_-e_l.png}
\caption{GNU-strings with UTF-16LE encoded input}
\label{fig:strings-e-l}
\end{figure}


\noindent \textbf{Results:} \texttt{UTF-8} is the only encoding in which \emph{GNU
strings} is able to find international characters.  The Figure
\ref{fig:strings-e-l}, chosen as an example, shows that with \texttt{UTF-16} no
international characters are recognized. The same holds true for \texttt{UTF-32}
...
characters in memory as 2 byte \texttt{UTF-16} words. As a result when dealing
...
\emph{GNU-strings} is not designed to analyse multi-byte encodings in general.
This is why other very common encodings e.g. \emph{big5} or \emph{koi8-r} can
not be tested even though they are widely used. The above-outlined limitations
leads to \emph{Strings­ext}'s main requirement: \emph{character encoding
support}.
...
In the Section \ref{sec:tool-requirements} we identified that the requirements
\emph{code efficiency} and \emph{security} must be accounted as of primary
importance. This section illustrates how Rust supports these requirements with
its \emph{zero cost abstractions} and its \emph{guaranteed memory safety}
leading finally to the choice of implementing \emph{Strings­ext} in Rust.
...
\cite{getreu2016a}\cite{getreu2017}. The technical challenges such as concurrent
batch processing of multi-byte character streams are sufficiently complex to
deduce general recommendations for forensic-tool development.
...
\begin{figure}[htbp]
\centering
\includegraphics[height=0.20000\textwidth]{images/stringsext-utf-16le-input-cut.png}
...
the scanners \emph{ASCII} and \texttt{UTF-8} find strings at the same location
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.450000\textwidth]{images/non-reproducible-output.png}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.450000\textwidth]{images/reproducible-output.png}
...
these requirements: Rust, as system
...
was the only time I encountered a bug in any of the used libraries. One
...
opinion, I largely prefer accepting the greater likelihood of manageable bugs
...
good choice for the present project, even though batch processing of
...
people's C code. Do I free the memory at the right moment? Is this pointer still
...
Getreu, J.: Stringsext, a {{GNU Strings Alternative}} with
  {{Multi}}-{{Byte}}-{{Encoding Support}} (Jan 2016),
