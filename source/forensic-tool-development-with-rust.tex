...
named \emph{Strings­ext}\cite{getreu2017a}\cite{getreu2017} serves as a case
...
development life-cycle have been executed.
...
memory. Even when the user does not directly communicate with the endpoint
device, traces may be found in all intermediate (network) devices. Due to the
cross-linked nature of computer systems the data that needs to be taken into
consideration when investigating a crime is enormous. In this huge amount of
information the investigator has to find those specific bits of information
constituting digital evidences. 
...
that it allows to reuse the test environment thus reducing the costs. Black box
...
the new tool \emph{Stringsext} deals correctly with big real-world input data:
\emph{Stringsext} has been designed to produce, as a special case, bit-identical
output to that of \emph{GNU-strings}.  This way the comparison of the output
data of both tools allowed to confirm that \emph{stringsext} is working
correctly when dealing with big data. 
...
you'' \cite{berinato2007}. This saying describes with striking simplicity the
relation between the criminal and the forensic investigator.  In digital
forensics this "hide-and-seek" relation might soon take a new dimension:
Eggendorfer \cite{eggendorfer2016} stresses with good reasons that forensic
tools are software too and therefor vulnerable to attacks. A vulnerability found
in 2017 in a common forensic tool \emph{EnCase Forensic Imager} demonstrates
exemplary the pertinence of the risk \cite{secconsult2017}: 
...
\cite{getreu2017a}\cite{getreu2017}. The technical challenges such as concurrent
