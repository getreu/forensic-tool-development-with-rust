...
Figures \ref{fig:strings-e-capitals} and \ref{fig:strings-e-l} and show
\emph{GNU-strings} output for 2 encodings as an example.

\begin{figure}[htbp]                                                             
\centering                                                                       
\includegraphics[width=0.80000\textwidth]{images/strings_-e_S.png}               
\caption{GNU-strings, single-8-bit option}                                       
\label{fig:strings-e-capitals}
\end{figure}  
...
\noindent
\textbf{Results:} As shown in the Figure \ref{fig:strings-e-capitals},
...
The Figure \ref{fig:strings-e-l}, chosen as an example, confirms that with
\emph{UTF-16} no international characters are recognized. The same holds true
for \emph{UTF-32} and most other encodings: This limitation is of particular
importance in forensic investigations: The Microsoft-Windows operating system
handles Unicode characters in memory as 2 byte \emph{UTF-16} words. As a result
when dealing with Microsoft-Windows memory images, \emph{GNU-strings} is not
able to detect any international characters!
It should not be forgotten that \emph{GNU-strings} can not analyse multi-byte
encodings in general. This is why other very common encodings e.g. \emph{big5}
or \emph{koi8-r} were not tested even though they are widely used. The
above-outlined limitations led to \emph{Strings­ext}'s main requirement: 
\emph{character encoding support}.
