...
well-known cause-effect-relationship between this observation (effect) and the
human action causing it, is a so called \textbf{artefact}.  \footnote{For the
sake of simplicity in this introduction, we present a rather simplistic view
associating univocally one trace (artefact/observation) with only one possible
cause/activity. In the physical world, one trace/observation has normally
several possible causes which cannot be excluded a priori. For this reason
modern forensics favours the \emph{Likelihood Ratio} approach, in which the
degree of support of the observations for the hypotheses is considered,
providing a strength of evidence (diagnostic value) that is relative to the
hypotheses tested.}
It embodies any ``item of
...
The latter aspect includes the so called \emph{string search} which is useful
...
they found you'' \cite{berinato2007}. Have you recognized the statement? Is
