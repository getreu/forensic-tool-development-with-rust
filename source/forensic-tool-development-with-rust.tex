...
characters in memory as 2 byte UTF-16 words. Thus, it is a common usecase in
forensics to search for foreign UTF-16 encoded scripts in memory images.
Unfortunately scanning for strings in UTF-16 is difficult, because almost every
random byte combination is assigned to a valid Unicode character. This leads to
many false positives and the result becomes unusable.  The offered solution by
\emph{Strings­ext} is to allow the user to restrict the Unicode-range when
searching. For example the Unicode-range \verb|U+400-U+7FF| captures only
strings in Cyrillic, Armenian, Hebrew, Arabic and Syriac. This feature is
...
read-only memory pages, which are then fed as input to the different
...
than expected. As we want to keep the scanning process as stateless as
possible, we introduced overlapping windows: This allows to read over the memory
page's edge in case a found string terminates in the next memory page.
However, strings can be so long that you can't but cut them somewhere. In this
case we need to make sure not to cut in the middle of a multi-byte character
which can be up to 6 bytes long. As a result a scanner is not completely 
stateless: between each scanner run the position where the last run stopped 
\verb|offset| and a flag indicating the forced cut of a string 
\verb|completes_last_string| is passed. 

A \emph{scanner} decodes the input stream in two phases: first it uses the 
...
extracts substrings while meeting additional criteria, e.g. minimum length or
Unicode-range restriction rules.
